...
class ErrorsHistogram
{
    public static function hist($s) {
        $search = ['u', 'w', 'x', 'z'];
        $hist = '';
        foreach ($search as $c) {
          $n = substr_count($s, $c);
          if ($n>=1) {
            $hist .= "$c  $n     ".str_repeat('*', $n)."\r";
          }
        }
        return trim($hist);
      }
...
// aaabbbbhaijjjm meaning that the printer used three times color a, four times color b, one time color h
...
// You have to write a function hist which given a string will output the errors as a string representing a
// histogram of the encountered errors.

// Format of the output string:

// letter (error letters are sorted in alphabetical order) in a field of 2 characters, a white space, number of
// error for that letter in a field of 6, as many "*" as the number of errors for that letter and "\r" (or "\n"
